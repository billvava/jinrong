	...
	placeholder:function()
	{
		try{
			if (window.qrh_Config && (window.qrh_Config.page=='publish'))
			{
				...
				{
					case '1':
						fields = {'i_overview':{'tip':'项目背景+项目介绍+融资需求+详细用途','lenerr':"必须大于50个并小于1000个汉字",'rule':'required|min_length[50]|max_length[1000]','maxlength':1000},
								'title':{'tip':'标准：地区+某行业项目+融资方式+金额（附单位）','holder':'举例：广东某食品公司生产项目股权融资10万-2000万元','rule':'required|min_length[8]|max_length[30]','lenerr':'请填写标题，8-30个汉字内'},
								'xmrz_usage':{'rule':'required|min_length[4]|max_length[30]','lenerr':'不少于4个字，末尾以句号结尾'}
								};
						break;
					case '200':
						fields = {'i_overview':{'tip':'请介绍具体内容（比如：专利、经营权、具体项目等情况）','lenerr':"必须大于50个并小于1000个汉字",'rule':'required|min_length[50]|max_length[1000]','maxlength':1000},
								'title':{'tip':'标准：地区+某资产主类型（公司/个人）+资产类别+转让方式+转让金额','holder':"举例：广东某个人专利整体转让20万元；浙江某公司风力发电项目整体转让500万元",'rule':'required|min_length[8]|max_length[30]','lenerr':'请填写标题，8-30个汉字内'}};
						break;
					case '700':
						fields = {'i_overview':{'tip':'项目介绍+招商需求+详细用途','lenerr':"必须大于50个并小于1000个汉字",'rule':'required|min_length[50]|max_length[1000]','maxlength':1000},
						'title':{'tip':'标准：地区+招商行业+开发区/区域+项目+政府招商','holder':'举例：湖北襄阳某高新技术开发区政府招商；广东某开发区食品生产项目政府招商','rule':'required|min_length[8]|max_length[30]','lenerr':'请填写标题，8-30个汉字内'}};
						break;
					case '2005':
						fields = {'i_overview':{'tip':'产品规模+年化收益+资金用途+还款来源+风控措施','lenerr':"必须大于50个并小于1000个汉字",'rule':'required|min_length[50]|max_length[1000]','maxlength':1000},
								'title':{'tip':'标准：地区+某公司+风险偏好+理财产品+产品投资门槛，预期收益','holder':'举例：广东某公司稳健型信托产品30万元起投，年化收益率9.7%','rule':'required|min_length[8]|max_length[30]','lenerr':'请填写标题，8-30个汉字内'}};
						break;
					case '2010':
					case '2011':
					case '2012':
					case '2013':
					case '2014':
						fields = {'i_overview':{'lenerr':"必须大于50个并小于1000个汉字",'rule':'required|min_length[50]|max_length[1000]','maxlength':1000},
								'title':{'tip':'标准：地区+某（资金主体/资金类型）+金额+寻某项目+合作','holder':'举例：广东某企业资金10万-2000万元寻全国优质房地产项目合作','rule':'required|min_length[8]|max_length[30]','lenerr':'请填写标题，8-30个汉字内'}};
						break;
					default:
						return;
				}
...
if (!window.qrh){
	window.qrh = new Object()
};
if (!window.qrh_Config) {
	window.qrh_Config = new Object()
};
function U(url)
{
	return '/'+url;
...
qrh.ui.tips = function(content, time){
	if ($.dialog)
	{
		$.dialog({
			id: 'Tips',
...
qrh.ui.confirm = function(content, yes, no){
	if (false && $.dialog)
	{
		$.dialog({
			id: 'Confirm',
...
qrh.ui.confirmb = function(content, yes, no){
	if ($.dialog)
	{
	$.dialog({
		id: 'Confirm',
...
qrh.ui.success = qrh.ui.error = qrh.ui.alert = qrh.ui.alertb = function(msg,yes){
	if($.dialog)
	{
	var html = '<div id="applyPrompt" class="hide p10" style="display: block;width:250px;">';
		html  += '<p class="pb10 ac">'+msg+'</p>';
		html  += '<p class="pt5 ac"><a href="javascript:;" onclick="qrh.cache.dialog.close();" class="gBtn22"><i class="gBtn22Inner">确 定</i></a></p>';
		html  += '</div>';
...
qrh.ui.dialog = function(self){
   ...
   success: function (data) {
	qrh.cache.dialog.content(data);
	qrh.cache.dialog.reset();
   }
   });
}


$.cookie = qrh.cookie = function(name, value, options) {
			   ...
			   if ( _rule.substring(0, 6) =='regexp' )
			   {
				...
				else if (_rule.substring(0, 9) == 'ajaxcheck')
				{
				   ...
				   var success = function(res){
					...
					if (qrh.cache[_val])
					{
						return success(qrh.cache[_val]);
					}
					qrh.Ajax.post("/api/reg/formcheck", "type="+_field+"&"+_field+"="+_val, success);
				}
			}
		   if (_ajaxcheck==false)$this.okhide == true ? $this.hidetip(self) : $this.tip(self, 'icoCor16');
...
function TrustOrder()
{
	var order = {
		...
		init:function(){
				...
				$('#'+self.prefix+'-submit').click(function(){
				   ...
				   var taname = $('#'+self.prefix+'-name'),
					...
					{
						 qrh.ui.alertb('请输入您的真实姓名');
						 taname.focus();
						 return;
					}
					if ( !qrh.Util.isChinese(_name))
					{
						 qrh.ui.alertb('请输入您的中文姓名');
						 taname.focus();
						 return;
					}
					if (!_mobile || !qrh.Util.isMobile(_mobile) )
					{
						 qrh.ui.alertb('请输入您的真实手机号码');
						 tamobile.focus();
						 return;
					}
					var param = 'type='+self.type+'&name='+_name+'&mobile='+_mobile+'&url='+self.url;
						param += '&message=&fromurl='+document.location.href;
					qrh.cache.submit = true;
					var _this = $(this);
					_this.html('正在提交中……');
					qrh.Ajax.post( '/api/guest/submit', param,function(res){
					   _this.html('立即预约');
					   qrh.cache.submit = false;
					   res.type='dialog';
						if (res.code)
						{
							var html = '<div class="subscribeDialog" id="subscribeDialog2">';
								 ...
								 html  += '<li><a href="javascript:;" onclick="qrh.cache.dialog.close();" class="yyBtnRed">确定</a></li>';
								 html  += '</ul></div>';
							try{
		...
		var html ='<div class="subscribeDialog" id="subscribeDialog">';
		   ...
		   $('a.T-online-order').click(function () {
			   qrh.cache.dialog = $.dialog({
				...
				self.url = window.qrh_Config.baseUrl+'/trust/detail_'+$(this).attr('data-id')+'.html';
				self.init();
	  ...
	  follow:function(item_id,callback){
			qrh.Ajax.post(U('service/click.trust'),'id='+item_id,function(res){
			if (res.code==200)
			{
			...
			{
				qrh.ui.alertb(res.data.message || '关注失败，请重试');
			}
		},function(){
			qrh.ui.error('网络异常，请重试');
		});
...
var MobileCode = {
	...
	voice:function(){
		 var self = this;
		 if(!self.mobile || self.mobile != self.mobileHand.val() || !self.smsid || qrh.cache.voice)return;
		 qrh.cache.voice = true;
		 qrh.Ajax.post('/api/mobile_regcode_voice','smsid='+self.smsid,function(res){
			 qrh.cache.voice = false;
			 if(res.code==200)
			 {
	...
	init:function(mobile){
		...
		$('#T-reg-'+self.mobileId+'-code').click(function(){
			...
			if (!_mobile_val || !qrh.Util.isMobile(_mobile_val))
			{
				return;
			}
			...
			qrh.Ajax.post("/api/mobile_regcode_send", "mobile="+_mobile_val, success, error);
		});
...
var oLogin={
	...
	login:function(){
			 ...
			 if (res.code == 110){
				 ...
				 else if (self.jump)
				 {
					 if (window.qrh_Config && window.qrh_Config.page=='publish')
					 {
					   ...
					   qrh.LoginID = res.data.login_user_id;
		...
		qrh.Ajax.post("/login/check", "_=&type=box&username="+login_username+'&password='+login_password+'&login_yzcode='+login_yzcode+'&is_auto_login='+is_auto_login+'&ver='+self.ver, success, error);
...
qrh.Login={
	...
	login:function(){
		...
		_this.find('i').html('正在登录中');
		qrh.Ajax.post("/login/check", "type=box&username="+login_username+'&password='+login_password+'&login_yzcode='+login_yzcode+'&is_auto_login='+is_auto_login+'&ver='+self.ver, success, error);
...
qrh.Login.dialog = function()
{
	qrh.cache.login_msg = qrh.Login.login_msg;
	qrh.Login.login_msg = qrh.Login.login_tip;
	qrh.cache.dialog = $.dialog({
		title: "请登录",
		lock:true,
		fixed:true,
		width:'480px',
		close: function(){
			qrh.Login.login_msg = qrh.cache.login_msg;
			qrh.Login.form = $('#TP-login');
			qrh.cache.dialog = null;
		}
	});
...
qrh.Login.lform = function(method,callback)
{
	qrh.cache.login_msg = qrh.Login.login_msg;
	qrh.Login.login_msg = qrh.Login.login_tip;
	qrh.cache.dialog = $.dialog({
...
qrh.Login.regform = function(method,callback)
{
	qrh.cache.dialog = $.dialog({
		lock:true,
		fixed:true,
...
qrh.Util = {
	...
	isMobile:function(mobile){
		return /^1(3[0-9]|4[0-9]|5[0-9]|7[0-9]|8[0-9])\d{8}$/.test( mobile );
	},
...
qrh.Ajax = {
	...
	post:function(url,param,callback_success,callback_error){
		qrh.Ajax.type = 'POST';
		qrh.Ajax.request(url,param,callback_success,callback_error);
	},
...
qrh.Core = {facebox:null}
qrh.Core.isLogin = function(){
	if (!qrh.LoginID)
	{
		qrh.Login.dialog();
		return false;
...
qrh.Core.enableLimitText = function(){
 ...
 * callback	回调函数
 */
qrh.Core.zjxmclick=function(id, callback){
qrh.Ajax.post(U('service/click.zjxmclick'),'id='+id,function(res){
		if (res.code==200)
		{
			if ( typeof callback == 'function' )
			{
			...
			qrh.ui.alertb('赞成功，感谢您的支持');
		}else
		{
			qrh.ui.alertb(res.data.message || '操作失败，请重试');
		}
	},function(){
		qrh.ui.error('网络异常，请重试');
	});
 ...
 * callback	回调函数
 */
qrh.Core.newsclick=function(id, callback){

	qrh.Ajax.post(U('service/click.newsclick'),'_&id='+id,function(res){
		if (res.code==200)
		{
			if ( typeof callback == 'function' )
			{
			...
			$('#click_num_'+id).text(parseInt($('#click_num_'+id).text())+1);
			qrh.ui.success('赞成功，感谢您的支持');
		}else
		{
			qrh.ui.alert(res.data.message || '操作失败，请重试');
		}
	},function(){
		qrh.ui.error('网络异常，请重试');
	});
...
qrh.Core.newscommentclick=function(id,type, callback){

	qrh.Ajax.post(U('service/click.newscommentclick'),'_=&id='+id+'&type='+type,function(res){
		if (res.code==200)
		{
			if ( typeof callback == 'function' )
			{
...
qrh.Core.activityclick=function(id, callback){

	qrh.Ajax.post(U('service/click.activityclick'),'id='+id,function(res){
		if (res.code==200)
		{
			if ( typeof callback == 'function' )
			{
 ...
 * callback	回调函数
 */
qrh.Core.answerclick=function(ask_id,content, callback){
	if ( ! qrh.Core.isLogin() )
	{
		return;
	}
	qrh.Ajax.post(U('service/common.answerclick'),'ask_id='+ask_id+'&content='+content,function(res){
		if (res.code==200)
		{
			if ( typeof callback == 'function' )
			{
			...
			//$('#click_num_'+id).text(parseInt($('#click_num_'+id).text())+1);
			qrh.ui.success('回答成功，待审核后就会显示出来');
		}else
		{
			qrh.ui.alert(res.data.message || '操作失败，请重试');
		}
...
qrh.Core.AddFavorite = function(){
	var url = 'http://www.7ronghui.com';
	var title = "金融网";
	if (document.all) {
...
qrh.Core.AddFavorite1 = function(){
	var url = 'http://www.7ronghui.com';
	var title = "金融网";
	try
	{
...
qrh.Core.toplogin = function(){
	var tplogin = $('#TP-login');
	if (tplogin.length==0)return;
	tplogin = new qrhLogin();
...
qrh.Core.topsearch = function(){
	...
	btn.click(function(){
		...
		if (_type)location.href='http://s' + qrh.cache.domain + '?act='+_type+'&k='+_keyword;
	});
 ...
 * toid 编号
 */
qrh.Core.zjxmrefresh = function(toid){
	if ( ! qrh.Core.isLogin() )return;
	qrh.cache.dialog = $.dialog({
		title: "信息更新确认",
		lock:true,
		fixed:true,
...
qrh.Core.newhand = function(self,act){
	if ( ! qrh.Core.isLogin() )return;
	qrh.Ajax.post('/service/common.newhand','act='+ act, function (res) {
 ...
 * source_id 来源id
 */
qrh.Evaluate.sendBox = function(infoid, fromtype,source,source_id){
	if ( ! qrh.Core.isLogin() )return;
	qrh.cache.dialog = $.dialog({
		title: "提交评价",
		lock:true,
...
qrh.Auth={}
qrh.Auth.sendBox = function(type,title){
	if ( ! qrh.Core.isLogin() )return;
	qrh.cache.dialog = $.dialog({
			title: title,
			lock:true,
...
qrh.Feedback.sendBox = function(toid){
	 if ( ! qrh.Core.isLogin() || qrh.cache.dialog)
	{
		return;
	}
	qrh.cache.dialog = $.dialog({
	   ...
	   $.ajax({
		...
		data:'id='+ toid,
		success: function (res) {
			if (res.code ==200)qrh.cache.dialog.content(res.data);
			if (res.code==500)qrh.cache.dialog.content('您的登录已失效，请刷新重新登录');
		},
		cache: false
	  });
}

qrh.Feedback.sendBoxV2 = function(toid){
	if ( ! qrh.LoginID )
	{
		qrh.Feedback.publishBox(toid,1);
		return;
	}
	if(qrh.cache.dialog)return;
	qrh.cache.dialog = $.dialog({
		title: "信息投递确认",
		lock:true,
		fixed:true,
	...
	$.ajax({
		...
		success: function (res) {
			if (res.code ==200)
			{
				qrh.cache.dialog.content(res.data);
				qrh.cache.dialog.reset();
			}else
			{
				qrh.cache.dialog.close();
				qrh.ui.alertb(res.data);
			}
			if (res.code==500)qrh.cache.dialog.content('您的登录已失效，请刷新重新登录');
		},
		cache: false
...
qrh.Feedback.publishBox = function(toid,type){
	if(qrh.cache.dialog)return;
	qrh.cache.dialog = $.dialog({
		title: "填写投递信息",
		lock:true,
...
qrh.Feedback.sendZjxm = function(toid){
	if(qrh.cache.dialog)return;
	qrh.cache.dialog = $.dialog({
		title: "信息投递确认",
		lock:true,
	...
	qrh.Ajax.post('/service/deliveryZjxm/fbox', 'zjxm='+ toid+'&type=1',function (res) {
		if (res.code ==200)
		{
			qrh.cache.dialog.content(res.data);
			qrh.cache.dialog.reset();
...
qrh.Feedback.sendCompany = function(toid){
	if(qrh.cache.dialog)return;
	 qrh.cache.dialog = $.dialog({
			title: "信息投递确认",
			lock:true,
	...
	qrh.Ajax.post('/service/deliveryCompany/fbox', 'company='+ toid+'&type=1',function (res) {
		if (res.code ==200)
		{
			qrh.cache.dialog.content(res.data);
			qrh.cache.dialog.reset();
...
//投递购物车
qrh.Carts={}
qrh.Carts.getCartNum = function(){
	$('#J_cartNum').text(qrh.cookie('TRJ_cartNum')||0);
}
qrh.Carts.sendBox = function(item_id,type,self){

	  qrh.Ajax.post('/service/cart.post', 'item_id='+item_id, function(res){
		if (res.code == 200)
		{
			if(type == 1)
			{
				...
				{
				   qrh.ui.alertb('成功加入投递箱，投递箱共'+res.data.count+'条数据');
				}
			}
			...
			qrh.ui.alertb(res.data.message);
		}
	})
}
 ...
 * touid 接收编号
 */
qrh.Message.sendBox = function(touid,toname){
	 if ( ! qrh.Core.isLogin() )
	 {
		  return;
	 }
 ...
 * callback	回调函数
 */
qrh.Message.send = function(message_to, message_content, callback){
	if ( ! qrh.Core.isLogin() )return;
	if(message_to == "") {
		qrh.ui.alert("收信人不能为空");
		return false;
	}
	if(message_content == "") {
		qrh.ui.alert("内容不可为空");
	...
	if (qrh.cache.sendMsg)return false;
	qrh.cache.sendMsg = true;
	qrh.Ajax.post(U('service/message.post'),{message_to:message_to, message_content:message_content},function(res){
		qrh.cache.sendMsg = false;
		if (res.code==200)
		{
			if ( typeof callback == 'function' )
			{
		...
		{
//			 if(res.code == -2){
//				  if(qrh.cache.dialog)qrh.cache.dialog.close();
//				  qrh.Auth.sendBox(3,'发私信');
//			 }else{
				var msg = '';
 ...
 * callback	回调函数
 */
qrh.Message.delMsg = function(id, callback){
	if(id == "") {
		qrh.ui.alert("请选择要删除的对话！");
		return false;
	}
	if ( ! qrh.Core.isLogin() )return;

	qrh.ui.confirm('确定要删除该条私信吗？', function () {
		 qrh.Ajax.post(U('service/message.delmsg'),{id:id},function(res){
			if (res.code==200)
			{
				if ( typeof callback == 'function' )
				{
 ...
 * callback	回调函数
 */
qrh.Message.delList = function(list_id, callback){
	if(list_id == "") {
		qrh.ui.alert("请选择要删除的私信！");
		return false;
	}
	if ( ! qrh.Core.isLogin() )return;
	qrh.ui.confirm('确定要删除该条私信吗？', function () {
		 qrh.Ajax.post(U('service/message.dellist'),{list_id:list_id},function(res){
			if (res.code==200)
			{
				if ( typeof callback == 'function' )
				{
 ...
 * callback	回调函数
 */
qrh.businesscard.get=function(callback){
	if ( ! qrh.Core.isLogin() )return;
	qrh.Ajax.post(U('service/businesscard.get'),'',function(res){
		if (res.code==200)
		{
 ...
 * callback	回调函数
 */
qrh.businesscard.exchange=function(uid, callback){
	if ( ! qrh.Core.isLogin() )
	{
		return
	};
	qrh.Ajax.post(U('service/businesscard.exchange'),'uid='+uid,function(res){
		if (res.code==200)
		{
			try{qrh.QP.baidu();}catch(err){}
			if ( typeof callback == 'function' )
			{
			   if (res.data.message)alert(res.data.message);
			   callback(res);return;
			}
			qrh.ui.success('交换名片申请成功');
		}else{
			if(res.code == -2){
				qrh.Auth.sendBox(2,'交换名片');
			}else{
				qrh.ui.alert(res.data.message || '交换失败，请重试');
		   }
		}
	},function(){
		qrh.ui.error('网络异常，请重试');
...
qrh.businesscard.bind = function(btn){
if (qrh.cache.businesscardbind)return;
qrh.cache.businesscardbind= true;
	 btn = btn || ".J-exchange";
	$(btn).live('click',function(){
		var _this   = $(this);
		qrh.businesscard.exchange( _this.attr('data-id'),function(res){
 ...
 * callback	回调函数
 */
qrh.businesscard.unfollow=function(uid, callback){
	 if ( ! qrh.Core.isLogin() )return;
	if (qrh.LoginID == uid)
	{
		alert('自己都不认识自己啦！');
	 ...
	 qrh.Ajax.post(U('service/businesscard.unfollow'),'uid='+uid,function(res){
		if (res.code==200)
		{
			if ( typeof callback == 'function' )
			{
			...
			qrh.ui.success('关注取消成功');

		}else
		{
			qrh.ui.alert(res.data.message || '关注取消失败，请重试');
		}
	},function(){
		qrh.ui.error('网络异常，请重试');
	});
 ...
 * callback	回调函数
 */
qrh.comment.zjxm=function(info_id, content,email_remind, callback){

	this.post('zjxm', info_id, content,email_remind, callback);
}

qrh.comment.news=function(info_id, content,email_remind, callback){
	this.post('news', info_id, content,email_remind, callback);
}

qrh.comment.zhiku=function(info_id, content,email_remind, callback){

	this.post('zhiku', info_id, content,email_remind, callback);
}

qrh.comment.post=function(type,info_id, content,email_remind, callback){
	 if ( ! qrh.Core.isLogin() ){
		 return;
	 }
	 email_remind = email_remind || 0;
	 qrh.Ajax.post(U('rest/comment/'+type),'_=&type='+type+'&item_id='+info_id+'&content='+content+'&email_remind='+email_remind,function(res){
		if (res.code==200)
		{
			try{qrh.QP.baidu();}catch(err){}
			if ( typeof callback == 'function' )
			...
			qrh.ui.success('评论发表成功，等待审核！');
		}else
		{
			qrh.ui.alert(res.data.message || '评论失败，请重试');
		}
	},function(){
		qrh.ui.error('网络异常，请重试');
	});
 ...
 * callback	回调函数
 */
qrh.follow.zjxm=function(item_id, callback){
	 if ( ! qrh.Core.isLogin() )
	 {
		  return;
	 }

	 qrh.Ajax.post(U('service/follow.zjxm'),'item_id='+item_id,function(res){
		if (res.code==200)
		{
			if ( typeof callback == 'function' )
			{
 ...
 * callback	回调函数
 */
qrh.follow.trust=function(item_id, callback){
	 if ( ! qrh.Core.isLogin() )
	 {
		  return;
	 }

	 qrh.Ajax.post(U('service/follow.trust'),'item_id='+item_id,function(res){
		if (res.code==200)
		{
			if ( typeof callback == 'function' )
			{
...
qrh.Reg = {
	mobileCode:function(mobile,method){
		...
		$('#T-reg-'+mobile+'-code').click(function(){
			...
			qrh.Ajax.post("/api/mobile_regcode_send", "mobile="+_mobile_val+'&method='+(method||''), success, error);
		});
...
qrh.Core.quick_pulish = function(){
	...
	$('#QS-getcode').click(function(){
		...
		var _this = $(this),_mobile_val=_mobile.val();
		if (!(_mobile_val.length == 11 && qrh.Util.isMobile(_mobile_val)))
		{
			...
			else if (res.code == 202)
			{
				...
				qrh.cache.qsmobile = _mobile_val;
				status = 'login';
				return;
			}
			else
		...
		qrh.Ajax.post("/api/reg/mobilecode", "mobile="+_mobile_val+'&method=publishquick', success, error);
	...
	els.blur(function(){
		...
		if(_id == 'QS-mobile' && status == 'login' && _val != qrh.cache.qsmobile)
		{
		...
		else if (self.attr('valid') == 'mobile' && !qrh.Util.isMobile(_val))
		{
	...
	$('#QS-submit').click(function(){
		...
		els.filter(':visible').each(function(){
			...
			else if ($(this).attr('valid') == 'username' && !new RegExp(regexEnum.ps_username, 'i').test(_val))
		   {
			...
			else if ($(this).attr('valid') == 'mobile' && !qrh.Util.isMobile(_val))
			{
				msg += '1';
				showerror(_id,'请输入真实的手机号码');
			}
		...
		var quick_save = function(){
			qrh.Ajax.post("/item_publish_quick/quick_save", param, function(res){
		...
		if(qrh.cache.qsloading)return;
		qrh.cache.qsloading = true;
		...
		if (status == 'login')
		{
			...
			var success = function(res){
				...
				{
					alert('您连续三次密码错误！');
					location.href=window.qrh_Config.baseUrl+'/login.html';
				}
			...
			qrh.Ajax.post("/login/check", "username="+_mobile_val+'&password='+_pwd+'&login_yzcode=&is_auto_login=0&ver='+qrh.Login.ver, success, error);
		...
		qrh.Ajax.post("/api/reg/submit", param, success, error);
});

}

qrh.Core.company_comment = function(){
  	if ( ! qrh.Core.isLogin() )return;
	$('#form').submit();
}

qrh.Core.quick_reg = function(){
	...
	$('#MP-submit').click(function(){
		...
		els.filter(':visible').each(function(){
			...
			else if ($(this).attr('valid') == 'chinese' && !qrh.Util.isChinese(_val))
			{
				msg += '1';
		...
		var error  = function(){
		 ...
		 var success  = function(res){
			   qrh.cache.mploading=false;
				if(res.code == 200)
				{
					location.href= window.qrh_Config.baseUrl+'/register/success.html';
				}
				else
				{
					if(typeof res.data.error_messages!='undefined')
...
qrh.Core.zjxm_view_reg = function(){
	...
	_mobile.change(function(){
		 qrh.cache.action = 'zjxm_view_reg';
		 $('#ZD-msg').hide();
		 if(qrh.cache.zdact == 'login')
		 {
			 $('#ZD-reg').show();
			 //$('#ZD-login').hide();
			 qrh.cache.zdact = 'reg';
	   ...
	   if (!_mobile.val() || !qrh.Util.isMobile(_mobile.val() ) )
	   {
			regfrm.tip(_mobile, 'icoErr16', '手机号码格式不正确');
			return;
	   }
	   if (qrh.cache.zdact == 'login')return;
		var success = function(res)
		{
			qrh.cache.zdclick = true;
			if (res.code == 200)
			{
		...
		qrh.Ajax.post("/api/reg/formcheck", "type=mobile&mobile="+_mobile.val(), success,function(){
			qrh.cache.zdclick = true;
		});
	});
	...
	_mobilecode.blur(function(){
	   ...
	   if (!_mobilecode.val() || _mobilecode.val() == _mobilecode.attr('tip'))
	   {
		...
		qrh.Ajax.post("/api/reg/formcheck", "type=mobilecode&mobile="+_mobile.val()+'&mobilecode='+_mobilecode.val(), success);
	});
	...
	$('#ZD-getcode').click(function(){
		...
		if (tt != 60 || process || qrh.cache.zdclick!==true)return;
		...
		qrh.Ajax.post("/api/reg/mobilecode", "mobile="+_mobile_val+'&from=zjxmdetail', success, error);
	...
	$('#ZD-submit').click(function(){
		...
		if (qrh.cache.zdloading)return;
		var param = '',msg = '';

		var error  = function(){
			 qrh.cache.zdloading=false;
			 loginmsg('网络异常，请重试！');
		 }
		 var success  = function(res){
			   qrh.cache.zdloading=false;
		...
		if (qrh.cache.zdact == 'login')
		{
			...
			qrh.cache.zdloading=true;
			qrh.Ajax.post("/login/check", "username="+_mobile_val+'&password='+_pwd_val+'&login_yzcode='+_yzcode_val+'&is_auto_login=0&ver='+qrh.Login.ver, success, error);
...
if (res.type && $.dialog)
{
	 if (res.type == 'dialoga' && res.code==200)
	 {
		...
		html  += '<p class="p10 ac"><a href="http://www'+domain+'/register.html" class="bBtn22"><i class="bBtn22Inner">立即注册</i></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:;" onclick="qrh.cache.dialog.close()" class="gBtn22"><i class="gBtn22Inner">取 消</i></a></p>';
		html  += '</div>';
		 qrh.cache.dialog = $.dialog({
			title:'申请提示',
			lock:true,
...
qrh.QP.baidu=function(){
$('body').append('<iframe src="'+window.qrh_Config.baseUrl+'/h/bd.html" style="display:none;"></iframe>');
}
qrh.QP.GJguest = function(){
	...
	qrh.Ajax.post( '/api/guest/submit', param,function(res){
	   qrh.cache.submit = false;
		...
		if (res.code==200)
		{
			...
			qrh.contact.ui('C-submit',0);
		}else if(res.code == 101){
			qrh.contact.ui('C-submit',1);
			qrh.cache.imgcode = '';
		}
	});
	...
	qrh.Ajax.post( '/api/guest/submit', param,function(res){
	   qrh.cache.submit = false;
		...
		if (res.code==200)
		{
			...
			qrh.contact.ui('GH-submit',0);
		}else if(res.code == 101){
			qrh.contact.ui('GH-submit',1);
			qrh.cache.imgcode = '';
		}
	});
...
qrh.QP.GGmessage = function(){
	...
	var html ='<div id="messageDialog">';
		...
		html +='<dl><dt>&nbsp;</dt><dd class="w270"><span class="fr mt5">已有账号<a href="http://www'+domain+'/login.html">请登录</a></span><a href="###" onclick="qrh.QP.GGsubmit()" class="bBtn34"><i class="bBtn34Inner">立即提交</i></a></dd></dl>';
		html +='</div>';
		html +='</div>';
	   $('a.message_dialog').live('click', function () {
		   qrh.cache.dialog = $.dialog({
			...
			qrh.Core.enableLimitText();
			qrh.cache.touid = $(this).attr('data-id');
		});
}
qrh.QP.GGsubmit = function()
{
	if (qrh.cache.submit)return;
	var taname = $('#GG-name'),
		 tamobile=$('#GG-mobile'),
	...
	var param = 'type=businesscard.umessage&name='+_name+'&mobile='+_mobile+'&url=';
		param += '&message='+_message+'&fromurl='+document.location.href+'&uid='+qrh.cache.touid;
	qrh.cache.submit = true;
	qrh.Ajax.post( '/api/guest/submit', param,function(res){
	   qrh.cache.submit = false;
		qrh.cache.dialog.close();
...
qrh.contact = {};
qrh.contact.ui=function(clickname,is_empty){
	var html = '';
	if(is_empty == 1){
		html = '<div id="exchangeCardDialog"><p class="pl10 pb5 clr6">您的网络申请过于频繁请输入验证码后再提交！</p><p class="p10"><input type="text" id="contact_code" value="" maxlength="4"><img onclick="this.src=\''+window.qrh_Config.baseUrl+'/util/captcha.html?\'+Math.random()" src="'+window.qrh_Config.baseUrl+'/util/captcha.html?0.8796358851250261" height="22"><a href="javascript:;" class="bBtn22" onclick="qrh.contact.submit(\''+clickname+'\');"><i class="bBtn22Inner">立即交换</i></a></p><p class="pl10 red" id="contact_code_error">验证码不正确</p></div>';
	}else{
...
qrh.contact.submit = function(clickname)
{
	qrh.cache.imgcode = $('#contact_code').val();
	if(qrh.cache.imgcode.length == 0){
...
qrh.Ajax.post( '/api/guest/submit', param,function(res){
	...
	if (res.code ==200)
	{
		...
		qrh.contact.ui('TA-submit',0);
	}else if(res.code == 101){
		qrh.contact.ui('TA-submit',1);
		qrh.cache.imgcode = '';
	}
});
});
...
qrh.Core.QApublish = function(pre){
...
$(function(){
	...
	{
	   qrh.Core.toplogin();//头部登录
	}
	$('#DY-submit').click(function(){
		var _email = $('#DY-email').val();
		if (_email && qrh.Util.isEmail(_email))
		{
			qrh.Ajax.post("/api/rss/email", 'email='+_email, function(res){
				if (res.code == 200)
				{
					alert('订阅成功！');
				}else if (res.code == 201)
				{
					alert('您输入的邮箱地址之前已订阅过金融网信息！');
				}
				else
				{
					alert('订阅失败，请重试！');
