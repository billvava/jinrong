...
class CompanyServiceController extends CompanyController{
	...
	public function index(){
		...
		$setmeal_list = D('Setmeal')->where(array('apply'=>1,'display'=>1))->select();
		foreach ($setmeal_list as $key => $value) {
			$setmeal_list[$key]['discount'] = D('Setmeal')->get_discount_for_setmeal_one($value);
			$setmeal_list[$key]['long'] = $this->format_days($value['days']);
		}
	...
	public function setmeal_add_save(){
        ...
        }else if($pay_type=='cash')
        {
	        ...
	        D('Order')->add_order(C('visitor'),$paysetarr['oid'],1,$setmeal_info['expense'],$paysetarr['ordtotal_fee'],$deductible,$setmeal_info['setmeal_name'],$payment_name,$paymenttpye['byname'],$description,$this->timestamp,1,0,$setmeal_id);
	        /* 会员日志 */
	        if($deductible>0 && $is_deductible==1){
	...
	protected function _increment_add_normal($cat){
		...
		foreach ($increment_arr as $key => $value) {
			...
			if($this->my_setmeal['is_free']==0){
				...
				$increment_arr[$key]['my_price'] = $discount>0?round($value['price']*$discount/10,1):$value['price'];
				//当前基础套餐的对应的折扣
				$increment_arr[$key]['my_discount'] = $discount;
				//单条价格
				$increment_arr[$key]['my_unit_price'] = round($increment_arr[$key]['my_price']/$value['value'],1);
				//节省的数
				...
				//免费会员需要付的价格
				$increment_arr[$key]['my_price'] = $free_discount>0?round($value['price']*$free_discount/10,1):$value['price'];
				//免费会员单条价格
				$increment_arr[$key]['my_unit_price'] = round($increment_arr[$key]['my_price']/$value['value'],1);
				//VIP会员价格,取出折扣最大的套餐折扣
				$vip_discount = D('Setmeal')->get_max_discount($cat);
				$increment_arr[$key]['vip_price'] = intval($vip_discount)>0?round($value['price']*$vip_discount/10,1):$value['price'];
				//VIP会员单条价格
				$increment_arr[$key]['vip_unit_price'] = round($increment_arr[$key]['vip_price']/$value['value'],1);
			}
			//换算积分
	...
	public function _increment_add_tpl($request_tpl=0){
		...
		foreach ($increment_arr as $key => $value) {
			...
			//当前基础套餐套餐需要付的价格,如果折扣为0,则价格与原始价格一致
			$increment_arr[$key]['my_price'] = $discount>0?round($value['tpl_val']/C('qscms_payment_rate')*$discount/10,1):($value['tpl_val']/C('qscms_payment_rate'));
			//当前基础套餐的对应的折扣
			$increment_arr[$key]['my_discount'] = $discount;
			//换算积分
	...
	public function _increment_add_save_download_resume(){
        ...
        $service_need_cash = $my_discount>0?round($increment_info['price']*$my_discount/10,1):$increment_info['price'];
	...
	public function _increment_add_save_tpl(){
        ...
        $service_need_cash = $my_discount>0?round($increment_info['tpl_val']/C('qscms_payment_rate')*$my_discount/10,1):$increment_info['price']/C('qscms_payment_rate');
	...
	protected function _call_cash_pay($increment_info,$order_pay_type,$payment_name='',$amount='0.0',$is_deductible,$deductible=0,$description='',$params='',$points=0,$stemeal=0,$discount=''){
        ...
        $paysetarr['oid']= strtoupper(substr($paymenttpye['typename'],0,1))."-".date('ymd',time())."-".date('His',time());//订单号
        D('Order')->add_order(C('visitor'),$paysetarr['oid'],$order_pay_type,$amount,$paysetarr['ordtotal_fee'],$deductible,$increment_info['name'],$payment_name,$paymenttpye['byname'],$description,$this->timestamp,1,$points,$stemeal,0,$params,$discount);
       	if($deductible>0 && $is_deductible==1){
        	$log_payment = $paymenttpye['byname'].'+'.C('qscms_points_byname').'抵扣';
        }else{
        	$log_payment = $paymenttpye['byname'];
    ...
    public function points_add_save(){
        ...
        else if($points>C('qscms_com_buy_points_max'))
        {
        	$points = C('qscms_com_buy_points_max');
        }
        
        $service_need_cash = round($points/C('qscms_payment_rate'),1);
    ...
    public function jobs_refresh(){
		...
		if(IS_POST){
			...
			if($jobs_num>1 && $jobs_num+$refresh_time>$this->my_setmeal['refresh_jobs_free']){
				$this->ajaxReturn(2,'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您共选中了 <span class="font_yellow">'.$jobs_num.'</span> 条职位，今天免费刷新次数已用完，请单条刷新。');
			}
			...
			if($mode=='points' && C('qscms_open_points_convert')==0){
				$mode = 'mix';
			}
			if($mode=='setmeal'){
				$show_footer = 1;
			...
			$this->assign('open_points_convert',C('qscms_open_points_convert'));
			if($jobs_num==1){
				$jobid = implode(",",$yid);
    ...
    public function jobs_refresh_all(){
    	...
    	if($user_jobs+$refresh_time>$this->my_setmeal['refresh_jobs_free']){
