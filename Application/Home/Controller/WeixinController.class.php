...
3.每日签到，免费获取".C('qscms_points_byname')."，商城好礼兑不停！";
	...
	protected function actionScan($object){
		...
		if($event_key<=10000000){
			...
			$usinfo = $this->get_user_info($object->FromUserName);
			if($usinfo){
				$this->content="您已绑定过".C('qscms_site_name')."帐号【".$usinfo['username']."】。如需解绑，请回复“解绑”";
			}else{
				$fromUsername = addslashes($object->FromUserName);
	...
	protected function clickBinding($object){
		$usinfo = $this->get_user_info($object->FromUserName);
		if($usinfo){
			$this->content="您已绑定过".C('qscms_site_name')."帐号【".$usinfo['info']['keyname']."】,如需解绑,请回复'解绑'";
		}else{
			$this->content="您还未绑定".C('qscms_site_name')."帐号，现在开始绑定：<a href='".$this->get_url(U('mobile/Members/apilogin_binding',array('openid'=>addslashes($object->FromUserName))))."'>点击开始注册/绑定帐号</a>";
		}
	}
			...
			if($usinfo['utype']!=2){
				...
				if(!$jobs_list['list']){
					...
					foreach ($jobs_list['list'] as $key => $value){
					    ...
					    $url = $this->get_url(U('mobile/jobs/show',array('id'=>$value['id'],'openid'=>addslashes($object->FromUserName))));
					    if($i==1){
		                    $picurl = $first_pic;
						}else{
...
您目前获得的总奖励为:".$all_points.C('qscms_points_byname');
			...
			$order=$sort.' desc';
            $key = get_tags($keyword);
            if(C('qscms_match_type') && $sort == 'score'){
                $where['key'] = array('match_with',$key);
            }
            // else
            // {
            //     $where['key'] = array('match_mode',$key);
            // }
            $field = 'id,uid';
            if($sort == 'score'){
                $field = 'id,uid,MATCH (`key`) AGAINST ("'.implode(' ',$key).'") as score';
            }
            if(C('qscms_jobs_display')==1){
				$where['audit'] = 1;
			}
            $mod = 'jobs_search_key';
            $jobs_search_list = D('JobsSearchKey')->field($field)->where($where)->order($order)->limit($limit)->select();
            
            $ids = array();  
            foreach ($jobs_search_list as $key => $val) {
                $ids[] = $val['id'];
                $uids[] = $val['uid'];
            }    
			if($ids){
				$fromUsername = addslashes($object->FromUserName);
				$first_pic = C('qscms_site_domain').C('qscms_site_dir').attach(C('qscms_weixin_first_pic'),'weixin');
				$jobs_list = M('Jobs')->where(array('id'=>array('in',$ids)))->limit(count($ids))->select();
				$company_list = M('CompanyProfile')->where(array('uid'=>array('in',$uids)))->limit(count($uids))->getfield('uid,logo');
				$i = 1;
				$content_arr = array();
				foreach ($jobs_list as $key => $val) {
				    $title = $val['jobs_name']."--".$val['companyname'];
				    $url = $this->get_url(U('mobile/jobs/show',array('id'=>$val['id'],'openid'=>$fromUsername)));
				    if($i==1){
	                    $picurl=$first_pic;	
					}else{
		                if ($picurl = $company_list[$val['uid']])
		                {
		                    $picurl=attach($picurl,'company_logo');
		                }
		                else
		                {
		                    $picurl=attach('no_logo.png','resource');
		                }
		                $picurl = C('qscms_site_domain').C('qscms_site_dir').$picurl;
					}
					$i++;
