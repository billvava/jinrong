...
class BackendController extends BaseController{
    ...
    protected function _list($model, $map = array(), $order='', $field_list='', $union='', $join=array(),$pagesize_by=10,$custom_fun){
        ...
        if ($pagesize) {
            ...
            if($union){
                $count = $model->query('select count(*) as tp_count from('.$count->buildSql().' union all '.$union.') as tp_t');
     ...
     * [pending 待处理事件统计]
     */
    protected function _pending($mod,$where){
        ...
        return M($mod)->where($where)->count();
    }
    ...
    protected function check_priv(){
        ...
        if(!session('admin') && !in_array(ACTION_NAME, array('login','verify_code'))) exit('<script type="text/javascript">top.location="'.U('index/login').'";</script>');
