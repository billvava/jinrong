...
class CompanyProfileModel extends Model
{
	...
	public function admin_edit_company_profile($data,$user){
		$this->_user = $user;
	...
	*/
		...
		if (fieldRegex($sqlin,'in'))
		{
			...
			if ($audit=="1" && $mailconfig['set_licenseallow']=="1")//认证通过
			{
				foreach ($comlist as $key => $value) {
					...
					if($user_info['email'] && $user_info['email_audit']=="1"){
						D('Mailconfig')->send_mail(array('sendto_email'=>$user_info['email'],'send_type'=>'set_licenseallow'));
					}
				}
			}
			if ($audit=="3" && $mailconfig['set_licensenotallow']=="1")//认证未通过
			{
				foreach ($comlist as $key => $value) {
					$user_info=D('Members')->get_user_one(array('uid'=>$value['uid']));
					if($user_info['email'] && $user_info['email_audit']=="1"){
						D('Mailconfig')->send_mail(array('sendto_email'=>$user_info['email'],'send_type'=>'set_licensenotallow'));
					}
				}
			}
					//sms
	...
	public function admin_edit_company_famous($uid,$famous,$reason,$audit_man=''){
		...
		{
			$comlist = $this->where(array('uid'=>array('in',$sqlin)))->select();
			if(false===$this->where(array('uid'=>array('in',$sqlin)))->setField('famous',$famous)) return false;
