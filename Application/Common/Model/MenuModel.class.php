...
class MenuModel extends Model {
    ...
    public function console_cache(){
        ...
        !APP_DEVELOPER && $where['mods'] = array('neq',1);
        $menus = M('Menu')->field('id,name,module_name,controller_name,action_name,data')->where($where)->order('ordid')->select();
    ...
    public function sub_menu_cache($pid = 0){
        ...
        $menuData = M('AdminAuth')->where($where)->join($join)->order('m.ordid,m.id')->getfield('m.id,m.name,m.menu_type,m.module_name,m.controller_name,m.action_name,m.data,m.stat,m.para');
