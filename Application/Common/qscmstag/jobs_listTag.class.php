...
class jobs_listTag {
    ...
    public function run(){
        ...
        if($jobs){
            ...
            $company_list = M('CompanyProfile')->where(array('id'=>array('in',$cids)))->limit(count($cids))->getfield('id,logo');
            if(C('apply.Subsite')){
            ...
            foreach ($jobs as $key => $val) {
                ...
                if ($this->params['brieflylen']>0){
                    $val['briefly']=cut_str(\Common\ORG\Input::deleteHtmlTags($val['contents']),$this->params['brieflylen'],0,$this->params['dot']);
                }else{
                    $val['briefly']=\Common\ORG\Input::deleteHtmlTags($val['contents']);
                }
                $val['amount']=$val['amount']=="0"?'若干':$val['amount'];
                $val['briefly_']=\Common\ORG\Input::deleteHtmlTags($val['contents']);
