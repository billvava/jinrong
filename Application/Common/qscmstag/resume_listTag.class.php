...
class resume_listTag {
    ...
    protected $default_order    = 'stime desc,refreshtime desc';    				//默认排序字段
    ...
    public function run($options){
        ...
        if($rids){
            $field = 'uid,id,display,strong_tag,stick,display_name,nature_cn,fullname,sex,major_cn,specialty,intention_jobs,trade_cn,photo,photo_img,photo_display,refreshtime,birthdate,tag_cn,talent,education_cn,sex_cn,wage_cn,experience_cn,district_cn,current_cn';
            ...
            foreach ($resume as $key => $val) {
				...
				if(in_array($val['id'],cookie('view_resume_log'))){
					$val['checked'] = true;
				}else{
					$val['checked'] = false;
				}
