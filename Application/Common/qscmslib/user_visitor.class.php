...
class user_visitor {
    ...
    public function __construct() {
        $this->_session = '_7rh'.md5(C('PWDHASH'));
        if (session('?'.$this->_session)) {
        	//已经登陆
        	$this->info = session($this->_session);
        	$this->is_login = true;
        }elseif ($user_info = (array)cookie($this->_session)) {
            $field = 'uid,utype,username,realname,mobile,last_login_time,last_login_ip,is_vip';
    ...
    public function login($uid,$remember=null) {
        ...
        $field = 'uid,utype,username,realname,mobile,password,last_login_time,is_vip';
        C('apply.Subsite') && $field.=',subsite_id';
    ...
    public function update(){
        $field = 'uid,utype,username,realname,password,mobile,last_login_time';
        C('apply.Subsite') && $field.=',subsite_id';
