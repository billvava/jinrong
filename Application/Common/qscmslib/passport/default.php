...
class default_passport
{
    ...
    public function auth($username,$password,$admin='') {
        ...
        $account_type_audit && $check_map[$account_type_audit] = array('eq',1);
        if($user = $mobile_mod->field('uid,password,pwd_hash,status')->where($check_map)->find()){
