...
class PersonalServiceController extends PersonalController{
    ...
    */
        ...
        if(IS_POST){
            ...
            if(!$return_order_info){
                ...
                if($rst['state']==1){
                    ...
                    /* 会员日志 */
                    write_members_log(C('visitor'),9001,$return_order_info['oid'],C('qscms_points_byname').'兑换');

                    $p_rst = D('MembersPoints')->report_deal($this->uid,2,$return_order_info['pay_points']);
                    if($p_rst){
                        /* 会员日志 */
                        write_members_log(C('visitor'),9002,$params['increment_name'],C('qscms_points_byname').'兑换');
            ...
            $resume_where['audit'] = array('neq',3);
            $resume_where['display'] = array('eq',1);
            $resume_list = D('Resume')->where($resume_where)->select();
            if($resume_id){
    ...
    public function order_detail($order_id){
        ...
        if($order['payment']=='wxpay' && $order['is_paid']==1 && $this->is_weixin && $points_enough){
            /**
             * 生成支付所需的参数
             */
