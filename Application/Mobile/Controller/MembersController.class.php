...
class MembersController extends MobileController{
        ...
        if(IS_POST){
            ...
            if($uid){
                ...
                if(!$login_url = cookie('return_url')){
                    $urls = array('0'=>'personal/index','1'=>'personal/index','2'=>'personal/index');
            ...
            if($this->visitor->is_login){
                cookie('return_url',null);
                $urls = array('1'=>'personal/index','2'=>'personal/index');
                $url = $s ? $return_url : U($urls[C('visitor.utype')]);
                $this->redirect($url);
            }
    ...
    protected function _correlation($data){
        ...
        if(false === $this->visitor->login($data['uid'])){
            ...
            $this->_404($this->visitor->getError(),'register');
        }
        if($data['openid']){
            $reg = \Common\qscmslib\weixin::bind($data['openid'],$this->visitor->info);
            if(!$reg['state']){
                IS_AJAX && $this->ajaxReturn(0,$reg['tip']);
                $this->_404($reg['tip'],'register');
            }
        }
        if($data['reg_type'] == 2){
    ...
    public function send_email_varify_url(){
        ...
        $url = build_mobile_url(array('c'=>'Members','a'=>'varify_email','params'=>'token='.$token));
        $to= $email;
        $subject=C('qscms_site_name').'邮件认证';
        $body=C('qscms_site_name').'提醒您：<br>您正在进行邮箱验证，该链接24小时内有效，点击链接完成验证：'.$url;
        if (true === $reg = company_send_mail($to,$name='', $subject = $subject, $body = $body)){
    ...
    public function varify_email(){
        ...
        $return_url_arr = array('1'=>U('Personal/index'),'2'=>U('Personal/index'));
        if($token){
            ...
            if($time+3600*24>=time()){//24小时内有效
                ...
                if(false === $reg = M('Members')->where(array('uid'=>$uid))->save($setsqlarr)) $this->_404('邮箱验证失败!',$return_url_arr[$userinfo['utype']]);
                if(!$reg){
    ...
    public function send_mobile_code(){
        ...
        $rand=mt_rand(100000, 999999);
        $sendSms = array('mobile'=>$mobile,'tpl'=>'set_mobile_verify','data'=>array('code'=>$rand));
        if (true === $reg = D('Sms')->sendSms('captcha',$sendSms)){
            session('verify_mobile',array('mobile'=>$mobile,'code'=>$rand,'time'=>time()));
    ...
    public function verify_mobile_code(){
        ...
        if (!$verifycode || !$verify['code'] || $verifycode<>$verify['code']) $this->ajaxReturn(0,'验证码错误!');
    ...
    public function user_getpass(){
        if(IS_POST){
            ...
            if($type == 1){
                ...
                if(!$user = M('Members')->field('uid,username')->where(array('mobile'=>$mobile,'mobile_audit'=>1))->find()) $this->_404('该手机号没有绑定帐号！');
                ...
                if(!$user = M('Members')->field('uid,username')->where(array('email'=>$email,'email_audit'=>1))->find()) $this->ajaxReturn(0,'该邮箱没有绑定帐号！');
                $time=time();
                $key=substr(md5($email.$time),8,16);
                $str = encrypt(http_build_query(array('e'=>$email,'k'=>$key,'t'=>$time)));
                $email_str.="{$user['username']}您好：<br>";
                $email_str.="请在24小时内点击以下链接重新设置您的密码：<br>";
                $url = C('qscms_site_domain').U('members/user_setpass',array('key'=>$str));
                $email_str.="<a href='".$url."' target='_blank'>".$url."</a><br>";
                $email_str.="如果链接无法点击,请复制粘贴到浏览器访问！<br>";
                $email_str.="本邮件由系统发出,请勿回复！<br>";
                $email_str.="如有任何疑问请联系网站官方：".C('qscms_top_tel');
    ...
    public function user_setpass(){
        if(IS_POST){
            ...
            $passport = $this->_user_server();
            if(false === $uid = $passport->edit($retrievePassword['uid'],$user)) $this->ajaxReturn(0,$passport->get_error());
            ...
            if($key){
                ...
                if(!$uid = M('Members')->where(array('email'=>$data['e']))->getfield('uid')) $this->_404('找回密码失败,帐号不存在!','user_getpass');
                $token=substr(md5(mt_rand(100000, 999999)), 8,16);
    ...
    public function apilogin_binding(){
        $this->_bind_info();
        $this->_config_seo(array('title'=>'帐号绑定 - '.C('qscms_site_name'),'header_title'=>'帐号绑定'));
        $this->display();
    }
    public function bind_reg(){
        $this->_bind_info();
        $this->_config_seo(array('title'=>'个人帐号注册 - '.C('qscms_site_name'),'header_title'=>'个人帐号注册'));
        $this->display();
    }
    public function bind_reg_com(){
        $this->_bind_info();
        $this->_config_seo(array('title'=>'企业帐号注册 - '.C('qscms_site_name'),'header_title'=>'企业帐号注册'));
        $this->display();
    }
    protected function _bind_info(){
        if($openid = I('get.openid','','trim')){
    ...
    public function oauth_reg(){
        ...
        $password = I('post.password','','trim');
        $passport = $this->_user_server();
        if(false === $uid = $passport->auth($username, $password)) $this->_404($passport->get_error());
        if(false === $this->visitor->login($uid)) $this->_404($this->visitor->getError());
        if($user_bind_info['type'] == 'weixin'){
            $openid = $user_bind_info['openid']?:I('request.openid','','trim');
            $reg = \Common\qscmslib\weixin::bind($openid,$this->visitor->info);
            ...
            if($bind_user['uid'] && $bind_user['uid'] != $uid) $this->_404('此帐号已经绑定过本站！');
            $user_bind_info['uid'] = $uid;
            if(false === $oauth->bindUser($user_bind_info)) $this->_404('帐号绑定失败，请重新操作！');
