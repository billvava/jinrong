    ...
    public function resume_down_confirm(){
        ...
        if($my_setmeal['download_resume_max']>0)
        {
            if($downnum>=$my_setmeal['download_resume_max']){
                $tip = '<div class="dialog_notice">您今天已下载 <span class="font_yellow">'.$downnum.'</span> 份简历，已达到每天下载上限，请先收藏该简历，明天继续下载。</div>';
                $mode = 'no';
        ...
        {
            if($my_points<C('qscms_download_resume_price')*C('qscms_payment_rate')){
                $this->assign('resume_id',$rid);
                $this->assign('discount',$discount);
                $tip = $this->fetch('Ajax_tpl/guide_pay_resume');
                $mode = 'mix';
            }else{
                $tip = '<div class="dialog_notice">您当前简历可用下载数为'.$my_setmeal['download_resume'].'，下载该简历需要扣除 <span class="font_yellow">'.intval(C('qscms_payment_rate')*C('qscms_download_resume_price')).'</span> '.C('qscms_points_byname').'，是否继续下载？<br /><div class="dialog_tip font10 font_gray9">您当前拥有 <span class="font_yellow">'.$my_points.'</span> '.C('qscms_points_byname').'</div></div>';
                $mode = 'points';
            }
        }else{
            $tip = '<div class="dialog_notice">你还可免费下载 <span>'.$my_setmeal['download_resume'].'</span> 份简历，是否继续下载？</div>';
            $mode = 'setmeal';
        }
        $this->ajaxReturn(1,$tip,$mode);
    }
