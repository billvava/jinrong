...
class SmsController extends ConfigbaseController{
    ...
    public function setup(){
        ...
        if(false === $tpl_list = F('sms_tpl')) $tpl_list = D('SmsTemplates')->sms_tpl_cache();
        if(false === $sms = F('sms_replace')){
            $sms = D('Sms')->sms_replace_cache();
        }
        $oauth = array('sid'=>$id,'create_time'=>time(),'update_time'=>time(),'status'=>1);
        $sms[$id]['filing'] && $oauth['status'] = 0;
        foreach ($tpl_list as $key => $val) {
            $oauth['tid'] = $val['id'];
            $oauth['alias'] = $val['alias'];
            $oauth['value'] = $val['value'];
            $sms[$id]['replace'] && $oauth['value'] = preg_replace('/\{(.*?)\}/i',$sms[$id]['replace'],$val['value']);
            $data[] = $oauth;
        }
        if(M('SmsOauth')->addAll($data)){
            $this->_mod->where(array('id'=>$id))->setfield('status',1);
    ...
    public function unload(){
        $id = I('get.id',0,'intval');
        !$id && $this->error('请选择短信服务商！');
        if(false !== M('SmsOauth')->where(array('sid'=>$id))->delete()){
            $this->_mod->where(array('id'=>$id))->setfield('status',0);
            $this->admin_write_log_unify($id);
            $this->success('卸载成功！');
        }else{
            $this->error('卸载失败！');
