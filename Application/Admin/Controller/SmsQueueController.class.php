...
class SmsQueueController extends BackendController{
    ...
    public function smsGroupSends(){
        $members_list = M('Members')->field('mobile')->where(['mobile'=>['neq',''],'inner'=>0])->select();
        //$item_members_list = M('Members')->field('mobile')->where(['utype'=>2,'mobile'=>['neq',''],'inner'=>0])->select();
        exit;
        $mobiles = array_multi_to_single($members_list);
        //$mobiles = [15938831629];
        foreach ($mobiles as $k => $v) {
            if(D('Sms')->sendSms('other',['mobile'=>$v,'tpl'=>'perfect'])){
                $setsqlarr['s_sendtime']=time();
                $setsqlarr['s_type']=2;//发送成功
                $setsqlarr['s_mobile']=$v;
    ...
    public function smsqueue_add(){
        if(IS_POST){
            ...
            foreach($mobile_arr as $key => $value){
                preg_match("/^(13|14|15|17|18)\d{9}$/",$value) && $m_arr[] = $value;
            }
            $num = 0;
            if(!empty($m_arr)){
    ...
    public function smsqueue_edit(){
        if(IS_POST){
            ...
            $s_body=I('post.s_body','','trim')?I('post.s_body','','trim'):$this->error('请填写短信内容');
            mb_strlen($s_body,'utf-8')>70 && $this->error('短信内容超过70个字，请重新输入！');
            if (preg_match("/^(13|14|15|17|18)\d{9}$/",$setsqlarr['s_sms'])){
                $_POST['s_type']=1;
                $_POST['s_body']=$s_body;
    ...
    public function smsqueue_batchadd(){
        if(IS_POST){
            ...
            foreach ($result as $key => $value) {
                if(preg_match("/^(13|14|15|17|18)\d{9}$/",$value['mobile'])){
    ...
    public function import_num(){
        ...
        $str = implode("|",$array);
        $str = trim($str,"|");
        $this->assign('import_numbers',$str);
