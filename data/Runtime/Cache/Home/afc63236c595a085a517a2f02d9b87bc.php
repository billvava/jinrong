    ...
    <div class="container">
        <div class="choose">
            <div class="result clearfix <?php if(select_condition()): else: ?>display-none<?php endif; ?>">
                ...
                <?php if($_GET['industry_id']!= ''): ?><a choose="industry" href="window.location='<?php echo P(array('key'=>''));?>';"><?php echo ($category['industry_id'][$_GET['industry_id']]); ?><i class="iconfont icon-error"></i></a><?php endif; ?>

                <?php if($_GET['province_id']!= ''): ?><a choose="region" href="window.location='<?php echo P(array('key'=>''));?>';"><?php echo ($category['province'][$_GET['province_id']]); ?><i class="iconfont icon-error"></i></a><?php endif; ?>

                <?php if($_GET['xmrz_type']!= ''): ?><a choose="financing-way" href="window.location='<?php echo P(array('key'=>''));?>';"><?php echo ($category['xmrz_type'][$_GET['xmrz_type']]); ?><i class="iconfont icon-error"></i></a><?php endif; ?>


                <?php if($_GET['xmf_zs_way']!= ''): ?><a choose="xmf_zs_way" href="window.location='<?php echo P(array('key'=>''));?>';"><?php echo ($category['xmf_zs_way'][$_GET['xmf_zs_way']]); ?><i class="iconfont icon-error"></i></a><?php endif; ?>

                <?php if($_GET['mo']!= ''): ?><a choose="financing-money" href="window.location='<?php echo P(array('key'=>''));?>';"><?php echo ($category['range'][$_GET['mo']]); ?><i class="iconfont icon-error"></i></a><?php endif; ?>
            ...
            <div class="xmf_zs_way">
                <span>招商方式：</span>
                <p choose="xmf_zs_way">
                    <a <?php if($_GET['xmf_zs_way']== ''): ?>class="select"<?php endif; ?> href="<?php echo P(array('xmf_zs_way'=>''));?>">不限</a>
                    <?php if(is_array($category['xmf_zs_way'])): $k = 0; $__LIST__ = $category['xmf_zs_way'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><a <?php if($_GET['xmf_zs_way']== $key): ?>class="select"<?php endif; ?> href="<?php echo P(array('xmf_zs_way'=>$key));?>"><?php echo ($vo); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
                </p>
            </div>
        </div>
                ...
                <?php if(!empty($info_list)): if(is_array($info_list)): $i = 0; $__LIST__ = $info_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
                    <div class="motif clearfix">
                        <div>
                            ...
                            <?php if($vo['amount_interval_min'] || $vo['amount_interval_max']): ?><p>融资资金： <span><?php echo ($vo["amount_interval_min"]); echo ($vo["amount_interval_min_unit"]); if($vo["amount_interval_max"] != ''): ?>-<?php echo ($vo["amount_interval_max"]); echo ($vo["amount_interval_max_unit"]); endif; ?>
                            </span></p><?php endif; ?>
                        </div>
                        <div>
                        <!--
                    ...
                    <div class="message">
                        ...
                        <?php if(!empty($vo['industry_id'])): ?><span><i class="iconfont icon-suoshuhangye"></i>所属行业：<?php echo ($vo["industry_id"]); ?></span><?php endif; ?>
                        <span><i class="iconfont icon-rongzi"></i>招商方式：<?php echo ($vo["xmf_zs_way"]); ?></span>
                    </div>
            ...
            <ul>
                <?php if(is_array($recommend_item)): $k = 0; $__LIST__ = $recommend_item;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><li>
                    <a class="recommend" href="<?php echo U('Item/show',array('id'=>$vo['id']));?>" target="_blank">
                        ...
                        <span>
                            <span>所在地区：<?php echo ($vo["province_id"]); ?></span>
