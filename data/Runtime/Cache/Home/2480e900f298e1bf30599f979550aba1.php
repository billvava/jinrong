...
<!--<link rel="stylesheet/less" type="text/css" href="less/item_list.less"/>-->
<!--<link href="../public/css/common.css" rel="stylesheet" type="text/css"/>-->
<!--<link href="../public/css/header.css" rel="stylesheet" type="text/css"/>-->
<!--<link href="../public/css/item.css" rel="stylesheet" type="text/css" />-->
<!--<link rel="stylesheet" href="../public/css/v2/item_list2.css">-->
    ...
    <div class="container">
        <div class="choose">
            <div class="result clearfix <?php if(select_condition()): else: ?>display-none<?php endif; ?>">
                ...
                <?php if($_GET['trade_category']!= ''): ?><a choose="trade_category" href="window.location='<?php echo P(array('trade_category'=>''));?>';"><?php echo ($category['trade_category'][$_GET['trade_category']]); ?><i class="iconfont icon-error"></i></a><?php endif; ?>

                <?php if($_GET['industry_id']!= ''): ?><a choose="industry" href="window.location='<?php echo P(array('industry_id'=>''));?>';"><?php echo ($category['industry_id'][$_GET['industry_id']]); ?><i class="iconfont icon-error"></i></a><?php endif; ?>

                <?php if($_GET['province_id']!= ''): ?><a choose="region" href="window.location='<?php echo P(array('province_id'=>''));?>';"><?php echo ($category['province'][$_GET['province_id']]); ?><i class="iconfont icon-error"></i></a><?php endif; ?>

                <?php if($_GET['trade_way']!= ''): ?><a choose="trade_way" href="window.location='<?php echo P(array('trade_way'=>''));?>';"><?php echo ($category['trade_way'][$_GET['trade_way']]); ?><i class="iconfont icon-error"></i></a><?php endif; ?>

                <?php if($_GET['mo']!= ''): ?><a choose="financing-money" href="window.location='<?php echo P(array('mo'=>''));?>';"><?php echo ($category['range'][$_GET['mo']]); ?><i class="iconfont icon-error"></i></a><?php endif; ?>

                </p>
                <span class="J_clear">清除 <i class="iconfont icon-clear"></i></span>
            </div>
            <div class="trade_category">
                <span>交易类别：</span>
                <p choose="trade_category">
                    <a <?php if($_GET['trade_category']== ''): ?>class="select"<?php endif; ?> href="<?php echo P(array('trade_category'=>''));?>">不限</a>
                    <?php if(is_array($category['trade_category'])): $k = 0; $__LIST__ = $category['trade_category'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><a <?php if($_GET['trade_category']== $key): ?>class="select"<?php endif; ?> href="<?php echo P(array('trade_category'=>$key));?>"><?php echo ($vo); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
                </p>
            ...
            <div class="trade_way">
                <span>交易方式：</span>
                <p choose="trade_way">
                    <a <?php if($_GET['trade_way']== ''): ?>class="select"<?php endif; ?> href="<?php echo P(array('trade_way'=>''));?>">不限</a>
                    <?php if(is_array($category['trade_way'])): $k = 0; $__LIST__ = $category['trade_way'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><a <?php if($_GET['trade_way']== $key): ?>class="select"<?php endif; ?> href="<?php echo P(array('trade_way'=>$key));?>"><?php echo ($vo); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
                </p>
            </div>
        </div>
        ...
        <div class="item_title clearfix">
            <div>
                ...
                <form id="item_form" action="Item/item_list">
                    ...
                    <input type="text" class="placeholder" tip="请输入搜索内容" value='<?php if($_GET['k']!= ''): echo ($_GET['k']); else: ?>请输入搜索内容<?php endif; ?>'>
                </form>
                </div>
            </div>
            <div>优质项目推荐</div>
            ...
            <ul>
                <?php if(!empty($info_list)): if(is_array($info_list)): $i = 0; $__LIST__ = $info_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
                    <div class="motif clearfix">
                        <div>
                            <h3><a href="<?php echo U('/Item/show',array('id'=>$vo['id']));?>" title="<?php echo ($vo["title"]); ?>" target="_blank"><?php echo ($vo["title"]); ?></a></h3>
                            <p>估价： <span><?php echo ($vo["xmzc_assass"]); echo ($vo["xmzc_assass_unit"]); ?></span></p>
                            <p>转让： <span><?php echo ($vo["transfer_price"]); echo ($vo["transfer_price_unit"]); ?></span></p>
                        </div>
                        <div>
                        <!--
                            <h4>姚先生</h4>
                    ...
                    <div class="message">
                        ...
                        <span><i class="iconfont icon-suoshuhangye"></i>交易类别：<?php echo ($vo["xmzc_type"]); ?></span>
                        <span><i class="iconfont icon-rongzi"></i>交易方式：<?php echo ($vo["trade_way"]); ?></span>
                    </div>
            ...
            <ul>
                <?php if(is_array($recommend_item)): $k = 0; $__LIST__ = $recommend_item;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><li>
                    <a class="recommend" href="<?php echo U('Item/show',array('id'=>$vo['id']));?>" target="_blank">
                        ...
                        <span>
                            <span>所在地区：<?php echo ($category[$vo['province_id']]); ?></span>
                            <span>所属行业：<?php echo ($vo["industry_id"]); ?></span>
                        </span>
                    </a>
