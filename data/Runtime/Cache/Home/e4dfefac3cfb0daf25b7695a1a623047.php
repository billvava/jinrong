<?php if (!defined('THINK_PATH')) exit(); $tag_help_show_class = new \Common\qscmstag\help_showTag(array('列表名'=>'info','id'=>$_GET['id'],'cache'=>'0','type'=>'run',));$info = $tag_help_show_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"{title} - {site_name}","keywords"=>"{title}","description"=>"{content}","header_title"=>""),$info);?>
<!doctype html>
<html>
<head>
    ...
    <div class="header_min_top">
        ...
        <div class="" style="width: 100px;float: left;height: 40px;color: #666666;line-height:40px;overflow: hidden;">
           <!-- <div class="leftimg"><img src="../public/images/11.png"></div> -->
            <?php $tag_notice_list_class = new \Common\qscmstag\notice_listTag(array('列表名'=>'notice_list','显示数目'=>'10','分类'=>'1','排序'=>'addtime:desc','cache'=>'0','type'=>'run',));$notice_list = $tag_notice_list_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"{title} - {site_name}","keywords"=>"{title}","description"=>"{content}","header_title"=>""),$notice_list);?>
    ...
    <div class="index_head">
        ...
        <div class="index_nav">
            <ul class="link_gray6 nowrap">
                <?php $tag_nav_class = new \Common\qscmstag\navTag(array('列表名'=>'nav','调用名称'=>'QS_top','数量'=>'10','cache'=>'0','type'=>'run',));$nav = $tag_nav_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"{title} - {site_name}","keywords"=>"{title}","description"=>"{content}","header_title"=>""),$nav);?>
...
<div class="helpshow">
  <div class="l">
	<div class="catbox">
	<?php $tag_help_category_class = new \Common\qscmstag\help_categoryTag(array('列表名'=>'category','大类'=>'0','cache'=>'0','type'=>'run',));$category = $tag_help_category_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"{title} - {site_name}","keywords"=>"{title}","description"=>"{content}","header_title"=>""),$category);?>
	<?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="titl"><?php echo ($vo['title_']); ?></div>
	  <ul class="link_gray6">
	  <?php $tag_help_category_class = new \Common\qscmstag\help_categoryTag(array('列表名'=>'subcate','大类'=>$vo['id'],'cache'=>'0','type'=>'run',));$subcate = $tag_help_category_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"{title} - {site_name}","keywords"=>"{title}","description"=>"{content}","header_title"=>""),$subcate);?>
  ...
  <div class="r">
    <div class="main">
  	  <div class="titl"><?php echo ($info['title']); ?></div>
      <div class="txt">
...
<div class="footer_min" id="footer">
	...
	<?php $tag_explain_list_class = new \Common\qscmstag\explain_listTag(array('列表名'=>'list','cache'=>'0','type'=>'run',));$list = $tag_explain_list_class->run();$frontend = new \Common\Controller\FrontendController;$page_seo = $frontend->_config_seo(array("title"=>"{title} - {site_name}","keywords"=>"{title}","description"=>"{content}","header_title"=>""),$list);?>
